...
$consulta = $pdo->prepare("SELECT DISTINCT da.id_doc_at,de.estatus,de.id_documento,de.folio,de.n_oficio,de.fecha_oficio,da.date_response,da.response,da.folio_response,dir.nombre_direccion,dep.nombre_departamento FROM documentos_externos as de INNER JOIN documentos_atendidos as da ON da.id_documento = de.id_documento INNER JOIN direccion as dir ON dir.id_direccion = da.id_dir_p INNER JOIN departamentos as dep ON dep.id_departamento = da.id_jc WHERE da.id_direccion_general = '$id_dir_general'");
$consulta->execute();
