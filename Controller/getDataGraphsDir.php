...
$number_days_month = cal_days_in_month(CAL_GREGORIAN, date("n"), 2022); // 31
...
$json5 = array();
for ($j = 0; $j < $days; $j++) {
    $date_data = $pdo->prepare("SELECT COUNT(res.id_documento) FROM documentos_externos as de INNER JOIN doc_ext_res as res ON res.id_documento = de.id_documento WHERE fecha_recibido = '$next_date[$j]' AND res.id_direccion = '$id_dir_gen'");
    $date_data->execute();
    $data_cont[$j] = $date_data->fetchColumn();
    //$json[] = array("Fecha" => $next_date[$i]);
    array_push($json2, $data_cont[$j]);

    $date_data_turn = $pdo->prepare("SELECT COUNT(di.id_documento) FROM documento_instruccion as di INNER JOIN documentos_externos as de ON di.id_documento = de.id_documento WHERE de.fecha_recibido = '$next_date[$j]' AND di.direccion = '$id_dir_gen'");
    $date_data_turn->execute();
    $data_cont_turn[$j] = $date_data_turn->fetchColumn();
    array_push($json3, $data_cont_turn[$j]);

    // SELECT COUNT(id_documento) FROM documentos_externos WHERE fecha_recibido = '$next_date[$j]' AND estatus = 305
    $data_response = $pdo->prepare("SELECT COUNT(id_documento) FROM documentos_atendidos as da WHERE da.date_response = '$next_date[$j]' AND da.id_direccion_general  = '$id_dir_gen'");
    $data_response->execute();
    $data_cont_r[$j] = $data_response->fetchColumn();
    array_push($json4, $data_cont_r[$j]);

    $data_limit = $pdo->prepare("SELECT Count(di.id_documento) FROM documento_instruccion as di INNER JOIN documentos_externos as de ON de.id_documento = di.id_documento WHERE di.direccion='$id_dir_gen' AND di.fecha_limite < CURDATE() AND de.fecha_recibido = '$next_date[$j]' ");
    $data_limit->execute();
